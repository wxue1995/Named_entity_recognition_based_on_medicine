...
def process_text(idx,split_method=None,split_name='train'):
    ...
    #-----------------------------提取词性和词边界特征BIEOS----------------------------------
    word_bounds=['M' for item in tag_list]#首先给所有的字都表上M标记
    ...
    def clean_word(w):
        ...
        if w.isdigit():#将所有的数字都变成一种符号
            return 'num' ##提高模型泛化能力
        return w
    dataset['word']=dataset['word'].apply(clean_word)##运用函数
    dataset.to_csv(save_path,index=False,encoding='utf-8')


##处理多个文件
def multi_process(split_method=None,train_ratio=0.8):
    if os.path.exists('data/prepare/'):#判断目录是否存在，
        shutil.rmtree('data/prepare/')#如果存在则删除
...
def mapping(data, threshold=10,is_word=False,sep='sep',is_label=False):
    count=Counter(data)#统计词频，返回列表
    if sep is not None:
        count.pop(sep) #删除sep
    if is_word:
        count['PAD']=100000001  #作为填充
        count['UNK']=100000000
...
def get_dict():
    ...
    for file in glob('data/prepare/train/*.csv')+glob('data/prepare/test/*.csv'):
        df=pd.read_csv(file,sep=',')##sep=‘,’分割符号
    ...
    with open(f'data/prepare/dict.pkl','wb') as f:
        pickle.dump(map_dict, f) #二进制储存数据
